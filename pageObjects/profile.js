...
expect(total).toStrictEqual(expectedTotal, 2);
 ...
 async checkout_Validate_Name_And_TotalPrice( firstName, lastName, postalCode) {
    ...
    await expect(this.page.getByText("Sauce Labs Backpack",{ exact: true })).toBeVisible();
    await expect(this.page.getByText("Sauce Labs Bolt T-Shirt",{ exact: true })).toBeVisible();
    await expect(this.page.getByText("Sauce Labs Bike Light",{ exact: true })).toBeVisible();
   ...
   async checkout_Validate_AddToCart_FirstItem_And_TotalPrice( firstName, lastName, postalCode) {
    ...
    await this.continueLocator().click();
    await this.page.waitForURL("**/checkout-step-two.html");
    await expect(this.page.getByText("Test.allTheThings() T-Shirt (Red)",{ exact: true })).toBeVisible();
    await this.totalPrice();
     await this.clickButton("Finish");
    await expect(this.successMsg).toBeVisible();
  }
